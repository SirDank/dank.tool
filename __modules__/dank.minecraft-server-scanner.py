...
def check(ip, server):
    try:
        ...
        try:
            ...
            server_info = "ipwho.is is unreachable"
...
def main():
    ...
    while True:
        ...
        if ips_amt.isdigit() and int(ips_amt) > 0:
            ips_amt = int(ips_amt)
            break
        rm_line()
    ...
    if server_type == "java":
        ...
        while True:
            ...
            if targeted_scan in ("0", "1"):
                targeted_scan = int(targeted_scan)
                break
            rm_line()
    else:
        targeted_scan = 0
        ...
        while generated < gen_amt:
            while True:
                try:
                    ...
                    input(clr(f"\n  > {translate('Failed to generate ips! Do not use [ Ctrl + C ]! Press [ENTER] to try again...')} ", 2))
                    rm_line()
